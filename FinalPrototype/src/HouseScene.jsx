...
import gameScene from './GameBoard';
...
class HouseScene extends Phaser.Scene {
  ...
  create(data) {
    ...
    const playerSprite = this.add.sprite(600, screenHeight/2, 'player');
    ...
    this.rexUI.add.label({
      ...
      width: 200,
      height: 40,
      text: this.add.text(500, 50, 'Welkom thuis!', { fontSize: '24px' }),
    });
    // Add your game objects to the scene here
    this.gameScene = this.scene.get('game');

    // const text = this.add.text(100, 100, 'You entered the house!', { fontSize: '32px', fill: '#fff' });

    // Add a close window button
    const closeButton = this.add.text(200, 50, 'X Terug', { fontSize: '32px', fill: '#fff' }).setInteractive();
    closeButton.on('pointerdown', () => {
     ...
     headButtonLeft.on('pointerdown', () => {
       ...
       playerSprite.setTexture(this.headArray[this.currentHeadIndex].default);
     });
     ...
     chestButtonLeft.on('pointerdown', () => {
       ...
       playerSprite.setTexture(this.chestArray[this.currentChestIndex].default);
     });
     ...
     legsButtonLeft.on('pointerdown', () => {
       ...
       playerSprite.setTexture(this.legsArray[this.currentLegIndex].default);
     });
  }
}
